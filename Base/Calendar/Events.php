...
class Base_Calendar_Events {
	...
	public function toArray() {
		...
		$end = $this->start->format('H:i:s');
		
		$dt = new Base_Controller_Action_Helper_Data();
		
		$end = $dt->adicionarHoras($end,1,'h',true);
		
		$end  = $this->start->format('Y-m-d')."T".$end."+00:00";
		
		$array['end'] = $end; //$end->format($format);
